...
def test_get_search_results_link():
    ...
    assert occurrences >= 12, f'{occurrences=} {search_term=}'
    assert len(result) >= 12, f'{len(result)=} {search_term=}'
    print(f'{occurrences=} {len(result)=} {search_term=}')
    
def test_get_episodes_list():
    anime_link = "https://witanime.cyou/anime/boruto-naruto-next-generations/"
